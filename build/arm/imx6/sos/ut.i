# 1 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c"
...
#define FASTPATH 1
# 1 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c"
# 11 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c"
...
# 12 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c" 2
...
extern seL4_Error cspace_ut_retype_addr(seL4_Word address,
                                        ...
                                        cspace_t *dest,
                                        seL4_CPtr *dest_cap);
# 13 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c" 2
...
# 15 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c" 2
...
#define CONFIG_LIB_ETHDRIVER_TX_DESC_COUNT 32
#define CONFIG_BUILDSYS_USE_CCACHE 1
# 17 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c" 2
...
# 20 "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c" 2
...
const seL4_BootInfo* _bi = ((void*)0);
...
static inline void _fill_table(const seL4_BootInfo *bi){
    ...
    ((i == _ut.count) ? ((void)0) : (__assert_fail("i == _ut.count", "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c", 48, __func__)));
}

static inline void _strip_min(int min_bits){
    int i, j;
    for(i = j = 0; i < _ut.count; i++){
        if((_bi->untypedSizeBitsList[_ut.map[i]]) >= min_bits){
            _ut.map[j++] = _ut.map[i];
        }
    }
    _ut.count = j;
...
static inline void _strip_max(int max_bits){
    int i, j;
    for(i = j = 0; i < _ut.count; i++){
        if((_bi->untypedSizeBitsList[_ut.map[i]]) <= max_bits){
            _ut.map[j++] = _ut.map[i];
        }
    }
    _ut.count = j;
...
static inline void _select_largest_contiguous(void){
    ...
    ((_ut.count) ? ((void)0) : (__assert_fail("_ut.count", "/home/mint/seL4/dhs-demo-feb-2018/apps/sos/src/ut_manager/ut.c", 90, __func__)));
    ...
    for(i = 0; i < _ut.count; i++){
        this_size += 1 << (_bi->untypedSizeBitsList[_ut.map[i]]);
        if(i + 1 >= _ut.count || (( _bi->untypedPaddrList[_ut.map[i]]) + (1 << ((_bi->untypedSizeBitsList[_ut.map[i]])))) != ( _bi->untypedPaddrList[_ut.map[i + 1]])){
...
static inline void _print_table(void){
    ...
    do { if ((0) < 0){ printf("\033[22;33m"); plogf("\n" "+-----------------------------------------------+\n" "|                 Untyped Table                 |\n" "|-----------------------------------------------|\n" "| ut(bi) |    start   ->    end     | size bits |\n" "|-----------------------------------------------|\n"); printf("\033[0;0m"); } } while (0)
                                                                             ...
                                                                             ;
    for(i = 0; i < _ut.count; i++){
        do { if ((0) < 0){ printf("\033[22;33m"); plogf("| %2d(%2d) | 0x%08x -> 0x%08x | %9d |\n", i, _ut.map[i], ( _bi->untypedPaddrList[_ut.map[i]]), (( _bi->untypedPaddrList[_ut.map[i]]) + (1 << ((_bi->untypedSizeBitsList[_ut.map[i]])))), (_bi->untypedSizeBitsList[_ut.map[i]])); printf("\033[0;0m"); } } while (0)
                                                                        ;
        if(i != _ut.count - 1 && (( _bi->untypedPaddrList[_ut.map[i]]) + (1 << ((_bi->untypedSizeBitsList[_ut.map[i]])))) != ( _bi->untypedPaddrList[_ut.map[i + 1]])){
            do { if ((0) < 0){ printf("\033[22;33m"); plogf("|-----------------------------------------------|\n"); printf("\033[0;0m"); } } while (0);
        }
    }
    do { if ((0) < 0){ printf("\033[22;33m"); plogf("+-----------------------------------------------+\n"); printf("\033[0;0m"); } } while (0);
...
int ut_translate(seL4_Word addr, seL4_Untyped* ret_cptr, seL4_Word* ret_offset){
    ...
    while(start < end){
        ...
        index = (start + end) >> 1;
        if(addr < ( _bi->untypedPaddrList[_ut.map[index]])){
            end = index;
        }else if((( _bi->untypedPaddrList[_ut.map[index]]) + (1 << ((_bi->untypedSizeBitsList[_ut.map[index]])))) <= addr){
            start = index + 1;
        }else{

            *ret_cptr = (_bi->untyped.start + _ut.map[index]);
            *ret_offset = addr - ( _bi->untypedPaddrList[_ut.map[index]]);
            return 0;
        }
    }
...
int ut_table_init(const seL4_BootInfo *bi){
    ...
    _low = ( _bi->untypedPaddrList[_ut.map[0]]);
    _high = (( _bi->untypedPaddrList[_ut.map[_ut.count - 1]]) + (1 << ((_bi->untypedSizeBitsList[_ut.map[_ut.count - 1]]))));
...
seL4_Word ut_steal_mem(int sizebits){
    ...
    paddr = ((((_low) + ((1 << sizebits) - 1)) & ~((1 << sizebits) - 1)));
